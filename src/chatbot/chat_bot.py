...
async def get_chat_history(session_id: str) -> list:
    ...
    try:
        if not session_id:
            logger.error(
                "[Session: None] Session ID is required to retrieve chat history"
            )
            return []

        chat_history = (
            await async_db.chat_history.find({"session_id": session_id}, {"_id": 0})
            .sort("timestamp", 1)
            .to_list(length=None)
        )

        logger.info(
            f"[Session: {session_id}] Found {len(chat_history) if chat_history else 0} messages in chat history"
        )
        ...
        logger.error(f"[Session: {session_id}] Error retrieving chat history: {e!s}")
        return []
...
async def save_intent_data(
    employee_id: str, session_id: str, intent_data: dict
) -> bool:
    logger.info(
        f"[Session: {session_id}] Saving intent data for employee: {employee_id}"
    )
    try:
        await async_db.intent_data.update_one(
            {"session_id": session_id, "employee_id": employee_id},
            {"$set": {"intent_data": intent_data, "updated_at": datetime.now(UTC)}},
            upsert=True,
        )
        logger.info(f"[Session: {session_id}] Successfully saved intent data")
        return True
    except Exception as e:
        logger.error(f"[Session: {session_id}] Error saving intent data: {e!s}")
        return False
...
async def get_intent_data(session_id: str) -> dict:
    ...
    try:
        if not session_id:
            logger.error(
                "[Session: None] Session ID is required to retrieve intent data"
            )
            return {}

        intent_data = await async_db.intent_data.find_one(
            {"session_id": session_id}, {"intent_data": 1, "_id": 0}
        )
        ...
        logger.error(f"[Session: {session_id}] Error retrieving intent data: {e!s}")
        return {}
...
async def save_conversation_status(session_id: str, status: str) -> bool:
    ...
    try:
        ...
        logger.error(f"[Session: {session_id}] Error saving conversation status: {e!s}")
        return False
...
async def get_conversation_status(session_id: str) -> str:
    ...
    try:
        ...
        logger.error(
            f"[Session: {session_id}] Error retrieving conversation status: {e!s}"
        )
        return "new"
...
async def extract_intent_from_employee(employee_profile, session_id: str):
    logger.info(
        f"[Session: {session_id}] Starting intent extraction from employee profile"
    )
    try:
        ...
        response = chat_model.chat.completions.create(
            ...
            messages=[
                ...
                {
                    "role": "user",
                    "content": f"Analyze the following employee profile comprehensively and provide the output in the JSON format specified above: Employee Profile: {employee_profile}",
                },
            ],
            temperature=0.7,
        ...
        logger.info(
            f"[Session: {session_id}] Successfully extracted intent data: {json.dumps(intent_data, indent=2)}"
        )
        return intent_data

    except json.JSONDecodeError as e:
        logger.error(
            f"[Session: {session_id}] JSON parsing error in intent extraction: {e!s} - Raw response: {response.choices[0].message.content}"
        )
        return None
    except Exception as e:
        logger.error(f"[Session: {session_id}] Error in intent extraction: {e!s}")
        return None


async def get_questions_from_tags(
    tags, tag_number=0, max_questions=10, session_id: str = None
):
    logger.info(
        f"[Session: {session_id}] Getting questions for tag number {tag_number}"
    )
    try:
        ...
        logger.info(
            f"[Session: {session_id}] Retrieved {len(questions) if questions else 0} questions for tag"
        )
        ...
        logger.error(f"[Session: {session_id}] Error in question extraction: {e!s}")
        return []
...
async def generate_next_question(
    intent_data: dict,
    chat_history: list,
    move_to_next_tag: int = 0,
    session_id: str = None,
) -> str:
    logger.info(
        f"[Session: {session_id}] Generating next question for tag index: {move_to_next_tag}"
    )
    try:
        reference_question = await get_questions_from_tags(
            intent_data["tags"], move_to_next_tag, session_id=session_id
        )

        messages = [{"role": "system", "content": QUESTION_GENERATION_SYSTEM_PROMPT}]

        question_number = 1

        if chat_history and len(chat_history) > 0:
            logger.info(
                f"[Session: {session_id}] Processing chat history with {len(chat_history)} messages"
            )
        ...
        logger.error(f"[Session: {session_id}] Error in question generation: {e!s}")
        return None
...
async def analyze_response(
    ...
    logger.info(
        f"[Session: {session_id}] Starting response analysis for tag: {current_tag}"
    )
    try:
        messages = [{"role": "system", "content": RESPONSE_ANALYSIS_SYSTEM_PROMPT}]
        ...
        logger.info(
            f"[Session: {session_id}] Analysis result: {json.dumps(analysis_result, indent=2)}"
        )
        return analysis_result

    except Exception as e:
        logger.error(f"[Session: {session_id}] Error in response analysis: {e!s}")
        return None
...
async def final_chat_analysis(session_id: str, chat_history, intent_data) -> dict:
    ...
    try:
        if not chat_history or not intent_data:
            logger.error(
                f"[Session: {session_id}] Missing chat history or intent data for final analysis"
            )
            return None
        ...
        messages = [
            {"role": "system", "content": FINAL_CHAT_ANALYSIS_SYSTEM_PROMPT},
            {
                "role": "user",
                "content": FINAL_CHAT_ANALYSIS_PROMPT.format(
                    intent_data=json.dumps(intent_data, indent=2),
                    conversation_history=json.dumps(formatted_conversation, indent=2),
                ),
            },
        ]
        ...
        try:
            ...
            if analysis_result.get("recommended_mentor") not in valid_mentors:
                raise ValueError(
                    f"Invalid mentor name: {analysis_result['recommended_mentor']}"
                )

            logger.info(
                f"[Session: {session_id}] Final analysis completed successfully"
            )
            return analysis_result

        except json.JSONDecodeError as e:
            logger.error(
                f"[Session: {session_id}] JSON parsing error in final analysis: {e!s}"
            )
            return None

    except Exception as e:
        logger.error(
            f"[Session: {session_id}] Error in final analysis: {e!s}", exc_info=True
        )
        return None


async def chat_complete(
    employee_id: str, session_id: str = None, message: str = None
) -> dict:
    logger.info(
        f"[Session: {session_id}] Starting chat completion for employee: {employee_id}"
    )
    try:
        ...
        if not chat_history or len(chat_history) == 0:
            ...
            intent_data = await extract_intent_from_employee(
                employee_profile, session_id
            )
            if not intent_data:
                logger.error(f"[Session: {session_id}] Failed to extract intent data")
                return {
                    "error": "Failed to analyze employee profile",
            ...
            question = await generate_next_question(
                intent_data, chat_history, move_to_next_tag=0, session_id=session_id
            )

            if not question:
                logger.error(
                    f"[Session: {session_id}] Failed to generate initial question"
                )
                return {
        ...
        if intent_data.get("chat_completed", False):
            logger.info(
                f"[Session: {session_id}] Mentor already assigned for this conversation"
            )
            return {
                "response": await mentor_chat_completion(
                    employee_id, intent_data, chat_history, session_id, message
                ),
        ...
        if (
            total_questions >= 10
            or not current_tag
            or number >= len(intent_data["tags"])
        ):
            logger.info(
                f"[Session: {session_id}] Conversation complete - Limits reached"
            )
            await save_conversation_status(session_id, "complete")
            final_analysis = await final_chat_analysis(
                session_id, chat_history, intent_data
            )
        ...
        analysis = await analyze_response(
            intent_data, chat_history, current_tag, total_questions, session_id
        )
        if not analysis:
        ...
        logger.info(
            f"[Session: {session_id}] Updating tag summary for {current_tag} and number {number}"
        )
        ...
        if number >= len(intent_data["tags"]):
            logger.info(
                f"[Session: {session_id}] Conversation complete - Analysis based"
            )
            await save_conversation_status(session_id, "complete")
            final_analysis = await final_chat_analysis(
                session_id, chat_history, intent_data
        ...
        next_question = await generate_next_question(
            intent_data, chat_history, move_to_next_tag=number, session_id=session_id
        )
        ...
        logger.error(
            f"[Session: {session_id}] Error in chat completion: {e!s}", exc_info=True
        )
        return {
