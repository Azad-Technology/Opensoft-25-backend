...
from src.chatbot.system_prompts import (
    ...
    RELATIONSHIP_SCORE_SYSTEM_PROMPT,
)
...
def tag_questions_batch(questions: list[str], api_manager) -> list[dict]:
    try:
        ...
        batch_content = (
            "Please analyze these questions and provide relevant tags for each:\n"
        )
...
def process_question_bank(
    question_bank: list[str], api_keys: list[str], batch_size: int = 10
):
        ...
        try:
            ...
            if result:
                ...
                for j, tagged_question in enumerate(result, 1):
                    ...
                    save_to_json(
                        question_data, "src/analysis/data/tagged_questions1.json"
                    )
...
def calculate_question_relationships_batch(
    question_a: str, other_questions: list[str], api_manager
) -> list[dict]:
    ...
    try:
        ...
        formatted_other_questions = "\n".join(
            [f"{i+1}. {q}" for i, q in enumerate(other_questions)]
        )
        ...
        for j in range(0, len(remaining_questions), batch_size):
            try:
                ...
                results = calculate_question_relationships_batch(
                    primary_question["question"], batch_questions, api_manager
                )
                ...
                if results:
                    ...
                    for k, result in enumerate(results):
                        ...
                        save_to_json(
                            relationship,
                            "src/analysis/data/question_relationships1.json",
                        )

                    print(
                        f"Processed relationships for question {primary_question['id']} with questions {batch[0]['id']} to {batch[-1]['id']}"
                    )

            except Exception as e:
                print(
                    f"Error processing batch for question {primary_question['id']}: {e}"
                )
                continue
...
if __name__ == "__main__":
    # Example usage:
    api_keys = [
        settings.GOOGLE_API_KEY1,
        settings.GOOGLE_API_KEY2,
        settings.GOOGLE_API_KEY3,
        settings.GOOGLE_API_KEY4,
        settings.GOOGLE_API_KEY5,
        settings.GOOGLE_API_KEY6,
        settings.GOOGLE_API_KEY7,
        settings.GOOGLE_API_KEY8,
        settings.GOOGLE_API_KEY9,
        settings.GOOGLE_API_KEY10,
        settings.GOOGLE_API_KEY11,
        settings.GOOGLE_API_KEY12,
        settings.GOOGLE_API_KEY13,
