...
class Neo4j:
    ...
    def get_related_questions(self, node_id, threshold):
        with self.driver.session() as session:
            result = session.execute_read(
                self._query_related_questions, node_id, threshold
            )
            return result
    ...
    def _create_node(tx, record):
        query = "CREATE (n:Question {id: $id, question: $question, tags: $tags})"
...
if __name__ == "__main__":
    # Opened csv files
    with open(question_bank, encoding="utf-8") as file:
        tagged_questions = json.load(file)

    with open(question_relations, encoding="utf-8") as file:
