...
async def start_chat(
    ...
    try:
        if session_id and not message.message:
            raise HTTPException(
                status_code=400, detail="Message is required for existing session"
            )
        ...
        logger.error(f"Error in start_chat: {e!s}")
...
async def get_session_history(
    ...
    try:
        ...
        session = await async_db.chat_history.find_one(
            {"session_id": session_id, "employee_id": current_user["employee_id"]}
        )
...
async def list_chat_sessions(current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        logger.error(f"Error listing chat sessions: {e!s}")
