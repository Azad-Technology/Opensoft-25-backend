...
async def get_employee_dashboard(employee_id: str, current_user: dict = Depends(get_current_user)):
    try:
        def process_doc(doc):
            ...
            for key, value in doc.items():
                if key == '_id':
                    ...
                    processed[key] = datetime.fromisoformat(value['$date'])
                elif isinstance(value, datetime):
                    processed[key] = value.isoformat()
                else:
                    processed[key] = value
        ...
        # Helper function to classify vibes remains the same
        def get_vibe(score):
            if not isinstance(score, (int, float)):
                return "unknown"
            if score > 4.5 and score <= 5:
        ...
        for leave in leave_data:
            ...
            if isinstance(leave_days, (int, float)):
                leave_counts[leave_type] += leave_days

        # Process leave data per month with proper date handling
        for leave in leave_data:
            leave_start_date = leave.get('Leave_Start_Date')
            if isinstance(leave_start_date, dict) and '$date' in leave_start_date:
                try:
                    leave_date = datetime.fromisoformat(leave_start_date['$date'])
                    if leave_date >= twelve_months_ago:
                        month_num = leave_date.month
                        month_name = months[month_num - 1]
                        year = leave_date.year
                        ...
                        leave_type = leave.get('Leave_Type', 'other')
                        leave_days = leave.get('Leave_Days', 0)
                        if not isinstance(leave_days, (int, float)):
                            leave_days = 0
                            
                        if leave_type == 'Sick Leave':
                            leave_per_month[key]['sick'] += leave_days
                        elif leave_type == 'Casual Leave':
                            leave_per_month[key]['casual'] += leave_days
                        elif leave_type == 'Annual Leave':
                            leave_per_month[key]['annual'] += leave_days
                        elif leave_type == 'Unpaid Leave':
                            leave_per_month[key]['unpaid'] += leave_days
                        else:
                            leave_per_month[key]['other'] += leave_days
        ...
        # (mental_states, weekly_activity, activity_stats, etc.)

        # Build comprehensive response
        response = {
            "employee_id": employee_id,
            "email": employee_email,
            "joining_date": process_doc(employee_data).get('Joining_Date', ''),
            "leaves": {
                "sick": leave_counts.get('Sick Leave', 0),
                "casual": leave_counts.get('Casual Leave', 0),
                "annual": leave_counts.get('Annual Leave', 0),
            ...
            # Include all other sections of your response...
        }
