...
from datetime import UTC, datetime  # Add timedelta to imports
...
async def get_employee_dashboard(
    ...
    try:
        ...
        recent_activity = (
            [a for a in activity_data if (datetime.now() - a["Date"]).days <= 3650]
            if activity_data
            else []
        )
        ...
        activity_stats = {
            "teams_messages_sent": int(
                sum(a["Teams_Messages_Sent"] for a in recent_activity)
            ),
        ...
        for activity in activity_data:
            if "Date" in activity and isinstance(activity["Date"], datetime):
                ...
                weekly_activity[day]["teams_messages_sent"] += activity.get(
                    "Teams_Messages_Sent", 0
                )
                weekly_activity[day]["emails_sent"] += activity.get("Emails_Sent", 0)
                weekly_activity[day]["meetings_attended"] += activity.get(
                    "Meetings_Attended", 0
                )
        ...
        avg_activity = {
            "teams_messages_sent": round(
                sum(a.get("Teams_Messages_Sent", 0) for a in activity_data)
                / len(activity_data),
                2,
            )
            if activity_data
            ...
            "emails_sent": round(
                sum(a.get("Emails_Sent", 0) for a in activity_data)
                / len(activity_data),
                2,
            )
            if activity_data
            else 0,
            "meetings_attended": round(
                sum(a.get("Meetings_Attended", 0) for a in activity_data)
                / len(activity_data),
                2,
            )
            if activity_data
            ...
            "work_hours": round(
                sum(a.get("Work_Hours", 0) for a in activity_data) / len(activity_data),
                2,
            )
            if activity_data
            else 0,
        ...
        for leave in leave_data:
            if "Leave_Start_Date" in leave and isinstance(
                leave["Leave_Start_Date"], datetime
            ):
        ...
        for perf in performance_data:
            if "Review_Period" in perf:
                ...
                month = perf["Review_Period"][
                    -7:
                ]  # Get last 7 chars (for "Jan-2023" format)
        ...
        latest_vibe = None
        if vibe_data:
            latest_vibe_data = sorted(
                vibe_data,
                key=lambda x: x.get("Response_Date", datetime.min),
                reverse=True,
            )[0]
            latest_vibe = {
        ...
        response = {
            ...
            "rewards": {
                ...
                "awards": [
                    r.get("Award_Type") for r in rewards_data if r.get("Award_Type")
                ],
            },
            "performance": process_doc(performance_data[0])
            if performance_data
            else None,
        ...
        raise HTTPException(
            status_code=500, detail=f"Error fetching employee dashboard: {e!s}"
        )
...
async def get_all_users(current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        onboarding_map = {
            o["Employee_ID"]: o for o in onboarding_data if "Employee_ID" in o
        }
            ...
            processed_users.append(
                {
                    ...
                    "onboarding_completed": onboarding.get(
        ...
        return JSONResponse(
            content={"count": len(processed_users), "users": processed_users}
        )

    except Exception as e:
        logger.error(f"Error fetching all users: {e!s}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"Error fetching user list: {e!s}")
    ...
    try:
        ...
        for v in vibe_data:
            if "Employee_ID" in v:
                ...
                if v["Employee_ID"] not in vibe_map or (
                    "Response_Date" in v
                    and (
                        v["Employee_ID"] not in vibe_map
                        or v["Response_Date"]
                        > vibe_map[v["Employee_ID"]].get("Response_Date", datetime.min)
                    )
                ):
        ...
        for p in performance_data:
            if "Employee_ID" in p:
                ...
                if p["Employee_ID"] not in performance_map or (
                    "Review_Date" in p
                    and (
                        p["Employee_ID"] not in performance_map
                        or p["Review_Date"]
                        > performance_map[p["Employee_ID"]].get(
                            "Review_Date", datetime.min
                        )
                    )
                ):
        ...
        for user in users_data:
            ...
            recent_activities = [
                a
                for a in activities
                if "Date" in a and (datetime.now() - a["Date"]).days <= 30
            ]  # Last 30 days

            activity_summary = {
                "teams_messages": sum(
                    a.get("Teams_Messages_Sent", 0) for a in recent_activities
                ),
                "emails_sent": sum(a.get("Emails_Sent", 0) for a in recent_activities),
                "meetings_attended": sum(
                    a.get("Meetings_Attended", 0) for a in recent_activities
                ),
                "avg_work_hours": round(
                    sum(a.get("Work_Hours", 0) for a in recent_activities)
            ...
            processed_users.append(
                {
                    ...
                    if onboarding.get("Onboarding_Completed")
                    else "Pending",
                    "last_activity": max(a["Date"] for a in activities).isoformat()
                    if activities
                    else None,
                    "current_vibe": get_vibe(latest_vibe.get("Vibe_Score"))
                    if latest_vibe
                    else None,
                    "performance_rating": latest_perf.get("Performance_Rating")
                    if latest_perf
                    else None,
        ...
        return JSONResponse(
            content={
                ...
                "users": sorted(
                    processed_users, key=lambda x: x["name"] if x["name"] else ""
                ),
            }
        )

    except Exception as e:
        logger.error(f"Error fetching detailed user list: {e!s}", exc_info=True)
        raise HTTPException(
            status_code=500, detail=f"Error fetching user details: {e!s}"
        )
...
async def add_onboarding(
    request: OnboardingRequest, current_user: dict = Depends(get_current_user)
):
    """
    ...
    try:
        ...
        if current_user["role"] != "hr":
            raise HTTPException(
                status_code=403, detail="Unauthorized to onboard employees"
            )

        # Check if employee_id already exists
        existing_employee = await async_db.users.find_one(
        ...
        logger.error(f"Error in onboarding: {e!s}")
        raise HTTPException(
            status_code=500, detail="An error occurred during onboarding"
        )
