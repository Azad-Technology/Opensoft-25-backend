...
from utils.config import get_async_database, settings
    ...
    try:
        ...
        return {
            ...
            "user": {
                ...
                "role": db_user["role"],
                "role_type": "admin"
                if db_user["email"] == "EMP0001@deloitte.com"
                else "employee",
        ...
        raise HTTPException(status_code=500, detail=f"Error in login: {e!s}")
...
async def get_access_token(current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        logger.error(f"Error in get_access_token: {e!s}")
        raise HTTPException(status_code=500, detail=f"Error in get_access_token: {e!s}")
