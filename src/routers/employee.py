...
async def get_vibescore_trend(time_period: str = "monthly", limit: int = 12):
    ...
    try:
        ...
        result = (
            df.groupby(group_col)["Vibe_Score"].agg(["mean", "count"]).reset_index()
        )
...
async def get_employee_performance(employee_id: str | None = None):
    """
    ...
    try:
        ...
        rewards_df = (
            pd.DataFrame(processed_rewards) if processed_rewards else pd.DataFrame()
        )
        ...
        if employee_id:
            result = {
                ...
                "promotion_consideration_rate": perf_df[
                    "Promotion_Consideration"
                ].mean(),
                "total_rewards_points": rewards_df["Reward_Points"].sum()
                if not rewards_df.empty
                else 0,
                "award_count": len(rewards_df) if not rewards_df.empty else 0,
                "performance_reviews": perf_df.replace({np.nan: None}).to_dict(
                    orient="records"
                ),
            }
        else:
            ...
            result = {
                ...
                "performance_rating_distribution": perf_df["Performance_Rating"]
                .value_counts()
                .astype(int)
                .to_dict(),
                "top_performers": perf_df.nlargest(5, "Performance_Rating")
                .replace({np.nan: None})
...
async def get_leave_analysis(leave_type: str | None = None):
    """
    ...
    try:
        ...
        result = {
            ...
            "monthly_leave_trend": df.groupby(
                df["Leave_Start_Date"].dt.strftime("%Y-%m")
            )["Leave_Days"]
            .sum()
            .to_dict(),
            "employees_with_most_leave": df.groupby("Employee_ID")["Leave_Days"]
            .sum()
            .nlargest(5)
            .to_dict(),
        }
...
async def get_onboarding_success():
    ...
    try:
        ...
        result = {
            ...
            "feedback_vs_training": pd.crosstab(
                df["Onboarding_Feedback"], df["Initial_Training_Completed"]
            ).to_dict(),
            "feedback_vs_mentor": pd.crosstab(
                df["Onboarding_Feedback"], df["Mentor_Assigned"]
            ).to_dict(),
        }
...
async def get_employee_activity(
    time_period: str = "daily", employee_id: str | None = None
):
    """
...
@router.get(
    "/employee-correlation", summary="Find correlations between different metrics"
)
async def get_employee_correlation():
    ...
    try:
        ...
        rewards_df = (
            pd.DataFrame(rewards_processed) if rewards_processed else pd.DataFrame()
        )

        # Aggregate data by employee
        employee_vibes = (
            vibes_df.groupby("Employee_ID")["Vibe_Score"].mean().reset_index()
        )
        employee_perf = (
            perf_df.groupby("Employee_ID")["Performance_Rating"].mean().reset_index()
        )

        employee_activity = (
            activity_df.groupby("Employee_ID")
            .agg(
                {
                    "Teams_Messages_Sent": "mean",
                    "Emails_Sent": "mean",
        ...
        if not rewards_df.empty:
            employee_rewards = (
                rewards_df.groupby("Employee_ID")["Reward_Points"].sum().reset_index()
            )
        else:
        ...
        return JSONResponse(
            content={
                ...
                "merged_metrics_sample": merged.head(5)
                .replace({np.nan: None})
                .to_dict(orient="records"),
            }
        )
...
async def get_total_employees():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error calculating total employees: {e!s}"
        )
...
async def get_employee_mood(time_period: str = "monthly", last_n: int = 3):
    ...
    try:
        ...
        mood_analysis = {
            ...
            "trend": "improving"
            if df.groupby("period")["Vibe_Score"].mean().is_monotonic_increasing
            else "declining",
        ...
        period_stats = (
            df.groupby("period").agg({"Vibe_Score": ["mean", "count"]}).reset_index()
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Error analyzing employee mood: {e!s}"
        )
    ...
    try:
        ...
        if activity_data:
            recent_activity = [
                ...
                if (datetime.now() - a.get("Date", datetime.now())).days <= 3650
            ]
        ...
        response = {
            ...
            "meetings_attended": int(
                sum(a.get("Meetings_Attended", 0) for a in recent_activity)
            ),
            ...
            "overall_activity_level": {
                "teams_messages_sent": int(
                    sum(a.get("Teams_Messages_Sent", 0) for a in recent_activity)
                ),
                "emails_sent": int(
                    sum(a.get("Emails_Sent", 0) for a in recent_activity)
                ),
                "meetings_attended": int(
                    sum(a.get("Meetings_Attended", 0) for a in recent_activity)
                ),
        ...
        raise HTTPException(
            status_code=500, detail=f"Error fetching employee summary: {e!s}"
        )
