...
class APIKeyManager:
    ...
    def get_next_available_key(self) -> str:
        ...
        while True:
            ...
            time_diff = (
                current_time - self.last_request_time[current_key]
            ).total_seconds()
            ...
            if (
                self.request_counts[current_key] < self.rate_limit
                and self.daily_counts[current_key] < self.day_limit
            ):
                # Key is available
    ...
    def use_and_get_key(self):
        ...
        logger.info(
            f"Using key {index} (Daily count: {self.daily_counts[key]}/{self.day_limit})"
        )
